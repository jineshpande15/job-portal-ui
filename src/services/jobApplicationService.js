...
export const getAppliedJobIds = async () => {
  try {
    // No dedicated ids endpoint -- derive from the application list.
    const response = await httpClient.get(API_ENDPOINTS.MY_APPLICATIONS);
    return (response.data || []).map((app) => app.jobId ?? app.job?.id);
  } catch (error) {
...
export const hasApplied = async (jobId) => {
  try {
    // No check endpoint -- derive from the application list.
...
export const getCompanyApplications = async () => {
  try {
    // No company-wide application endpoint exists on the backend; the employer
    // API is per-job (GET /jobs/applications/{jobId}/employer).
    throw new Error(
      "getCompanyApplications is not supported: use getApplicationsByJob(jobId) instead"
    );
  } catch (error) {
