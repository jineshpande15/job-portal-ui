...
export const fetchContactById = async () => {
  try {
    ...
    throw new Error(
      "fetchContactById is not supported: the backend exposes no GET /contacts/{id}"
    );
  } catch (error) {
...
export const updateContactStatus = async (id, status) => {
  try {
    const response = await httpClient.patch(
      API_ENDPOINTS.UPDATE_CONTACT_STATUS(id),
      { status }
    );
    return response.data;
  } catch (error) {
...
export const deleteContact = async () => {
  try {
    ...
    throw new Error(
      "deleteContact is not supported: the backend exposes no DELETE /contacts/{id}"
    );
  } catch (error) {
