...
export const fetchCompanyById = async (id) => {
  try {
    // The backend exposes no GET /companies/{id}; pull the public list and pick
    // the one we want rather than hitting an unmapped path (which 403s).
    const response = await httpClient.get(API_ENDPOINTS.COMPANIES);
    const company = response.data.find((c) => String(c.id) === String(id));
